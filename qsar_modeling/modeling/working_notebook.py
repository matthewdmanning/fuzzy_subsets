...
import numpy as np
import pandas as pd
from ITMO_FS.filters import MRMR
...
from constants import paths, names, run_params
from feature_selection.importance import extra_trees_gini_importances
from feature_selection.vif import calculate_vif
from feature_selection.vif import repeated_stochastic_vif
...
def train_models(train_X, train_y, total_meta_df):
    ...
    class_wts_arr = compute_class_weight(class_weight='balanced', classes=classes, y=train_y)
    ...
    sample_wts = pd.Series(data=compute_sample_weight(class_weight='balanced', y=train_y), index=train_y.index)
    ...
    if os.path.isfile(paths.cov_mat) and os.path.getsize(paths.cov_mat) > 10000:
        with open(paths.cov_mat, 'r+b') as f:
            ...
            if loaded_cov is not None and not loaded_cov.empty:
                pprint.pp('Covariance matrix has been loaded with shape: {}'.format(loaded_cov.shape))
            else:
    ...
    if reuse_fs and not run_params.debug and not fs_loaded and not fs.X.feat_frame.empty:
    ...
    if fs.X.cov_mat is None or (
            (type(fs.X.cov_mat) is not pd.DataFrame or fs.X.cov_mat.empty) or type(fs.X.cov_mat) is not np.ndarray):
        # print('Not using Cov?')
        pass
        # logger.info('Covariance matrix could not be calculated!!!')
    elif not os.path.isfile(paths.cov_mat) and (os.path.getsize(paths.cov_mat) == 0) and loaded_cov is None:
    ...
    bad_feats = pd.Index([c for c in std_cut_feats if any([u.lower() in c.lower() for u in names.uninterpretable])])
    ...
    def MRMR_proto(feats_df, sample_subset, n_feats_out, initial_feats=None, n_intial_feats=None,
    ...
    def mi_select(feats_df, target, feature_names_in, n_feats_out, save_dir, k=5, cv=30, n_pca_in=15, n_pca_out=15):
        ...
        if os.path.isfile('{}mutual_info.csv'.format(save_dir)):
            ...
            for rus_num in list(range(cv)):
                feats_under, label_under = RandomUnderSampler().fit_resample(feats_df, target)
                feats_under = feats_df.loc[feats_under.index]
                new_mi = mutual_info_classif(feats_under, label_under, n_neighbors=k, random_state=0, n_jobs=-1)
                mi_list.append(new_mi)
            # print('Length of mi_list: {}'.format(mi_list))
            mi_df = pd.concat([pd.Series(data=mi, index=feats_df.columns) for mi in mi_list], axis=1)
        ...
        pd.concat([mi_ser, mi_std[mi_ser.index]], axis=1, names=['Mean', 'Std']).to_csv(
        )
        ...
        for i in list(range(vif_loops)):
            vif_ser = calculate_vif(vif_all_feats, candidates, n_jobs=1, generalized=False)
            vif_ser.copy().transform(func=softmax, axis=0)
            mi_scores = mi_ser[vif_ser.index].copy().transform(func=softmax)[vif_ser.index]
        ...
        logger.info('Incremental PCA from MI output of top {} features, keeping:'.format(n_pca_in, n_pca_out))
        mi_pca = IncrementalPCA(n_components=15, whiten=True, batch_size=5000).fit(X=new_feats.iloc[:n_pca_in])
        ...
        with open('{}pca_{}_tp_{}.csv'.format(save_dir, n_pca_in, n_pca_out), 'w', encoding='utf-8') as f:
            pd.Series(mi_pca.explained_variance_).sort_values(ascending=False).to_csv(path_or_buf=f)
        plt.plot(mi_pca.explained_variance_)
        plt.show()
        return new_feats

    def vif_select(feats_df, target, feature_names_in, n_feats_out, save_dir, model_cv=100, model_size=20,
                   model_type='ols', sample_wts=samp_wts, n_jobs=-1, verbose=0, **kwargs):
        if uni_mi:
            uni_mi = mutual_info_classif(X=df, y=target, random_state=0, n_jobs=-1)
        vif_returns = repeated_stochastic_vif(feats_df[feature_names_in], mi_ser=uni_mi,
                                              target=target, num_select=n_feats,
                                              sample_wts=sample_wts, model_size=model_size,
                                              cv=model_cv,
                                              n_jobs=-1,
                                              verbose=verbose, **kwargs)
        # vif_criteria, vif_score_df, vif_stats, votes = vif_returns
        if save_dir is str:
            vif_paths = ('{}vif_mi_criteria.pkl'.format(save_dir),
    ...
    def extra_trees_select(feats_df, target, feature_names_in, n_feats_out, save_dir):
        feats_df = extra_trees_gini_importances(data_df=feats_df, labels=target,
                                                sample_wts=sample_wts, n_feats_out=n_feats_out, save_dir=save_dir)
        with open('{}extra_trees_feature_importance.csv'.format(save_dir), 'w', encoding='utf-8') as f:
            f.writelines([x for x in feats_df])
        return feats_df

    mi_select(feats_df=std_cut_feats[good_feats], target=fs.y, feature_names_in=good_feats.tolist(), n_feats_out=25,
              save_dir=os.environ.get(
                  'MODELS_DIR'))

    # TODO Implement Select-Train-Score refactor.
    outputs_keys = ['n_feats', 'feat_meth', 'n_cv_splits', 'results', 'models']
    first_run = {'initial_feats': good_feats, 'n_feats_out': [250, 50], 'feat_meth': ['disr', 'disr'],
                 'n_cv_splits':   5,
                 'results':       [], 'models': []}
    second_run = {'initial_feats': good_feats, 'n_feats_out': [100, 50], 'feat_meth': ['mi', 'extra'],
                  'n_cv_splits':   5,
                  'results':       [], 'models': []}
    all_feats_disr = {'initial_feats': std_cut_feats.columns, 'n_feats_out': [250, 50], 'feat_meth': [
        'disr', 'disr'],
                      'n_cv_splits':   5,
                      'results':       [], 'models': []}
    all_feats_mi_trees = {'initial_feats': std_cut_feats.columns, 'n_feats_out': [250, 50],
                          'feat_meth':     ['mi', 'extra'],
                          'n_cv_splits':   5,
                          'results':       [], 'models': []}
    ...
    def feature_selection(split_data, target, fs_params, sel_dir):
        ...
        for feat_num, feat_type in enumerate(fs_params['feat_meth']):
            select_dir = '{}{}_{}_{}features/'.format(sel_dir, feat_num, feat_type,
                                                      fs_params['n_feats_out'][feat_num])
            os.makedirs(select_dir)  # results = dict([(k, dict()) for k in fs_params['feat_meth'][feat_num]])
            if feat_type == 'cmim':
                pass
                # ITMO_FS.CMIM()
            if feat_type == 'disr':
                new_feats = ITMO_FS.filters.DISRWithMassive(
                    expected_size=fs_params['n_feats_out'][feat_num]).fit_transform(
                    X=new_feats, y=target, feature_names=new_feats.columns)
            ...
            if feat_type == 'mi':
                ...
                new_feats = mi_select(new_feats, target, feature_names_in=new_feats.columns, n_feats_out=fs_params[
                    'n_feats_out'][feat_num], save_dir=select_dir)
                '''
                ...
                '''
            if feat_type == 'extra':
                new_feats = extra_trees_select(new_feats, target, new_feats.columns,
                                               n_feats_out=fs_params['n_feats_out'][
                                                   feat_num], save_dir=select_dir)
            ...
            if feat_type == 'fmim':
                ...
                fmim_scores = fmim(features=fs.X.feat_frame[fmim_feats_in], label=fs.y,
                                   num_feats=fs_params['n_feats_out'][feat_num],
                                   n_jobs=run_params['n_jobs'])
                first_feats = pd.Series(data=fmim_scores, index=fmim_feats_in).sort_values(ascending=False)
    ...
    for run_num, out in enumerate(output_list):
        ...
        results = dict([(k, dict([(s, list()) for s in split_keys])) for k in model_keys])
        ...
        for d, e in StratifiedKFold().split(X=std_cut_feats, y=fs.y):
            ...
            feats_df_list.append(feature_selection(std_cut_feats.iloc[d], fs.y.iloc[d], fs_params=out,
                                                   sel_dir=fs_dir))
        ...
        for mod_name in model_keys:
            ...
            def run_balanced_tree(n_trees=1000):
                logger.info('Training Balanced Random Forest with {} trees'.format(n_trees))
                rf_path = '{}imb_rf_{}_cv_{}.pkl'.format(CV_DIR, '-'.join([str(x) for x in out['n_feats_out']]), cv)
                brfc = BalancedRandomForestClassifier(n_estimators=n_trees, max_depth=25, n_jobs=-1, random_state=0,
                ...
                with open(rf_path, 'w+b') as f:
                    pickle.dump((models['rf'][cv], (indices['dev'][cv], indices['eval'][cv])), file=f)
                results['rf']['dev'].append(models['rf'][cv].predict(X_dev))
                results['rf']['eval'].append(models['rf'][cv].predict(X_eval))
                logger.info(
                    'Matthews Correlation Coefficient (Dev): {}:'.format(matthews_corrcoef(y_true=results['true'][
                        'dev'][cv],
                                                                                           y_pred=results['rf'][
                                                                                               'dev'][cv])))
                logger.info(
                    'Matthews Correlation Coefficient (Eval): {}:'.format(matthews_corrcoef(y_true=results['true'][
                        'eval'][cv],
                                                                                            y_pred=results['rf'][
                                                                                                'eval'][cv])))
                logger.info(
                    'Balanced Accuracy (Dev): {}:'.format(balanced_accuracy_score(y_true=results['true']['dev'][cv],
                                                                                  y_pred=results['rf'][
                                                                                      'dev'][cv])))
                logger.info(
                    'Balanced Accuracy (Eval): {}:'.format(balanced_accuracy_score(y_true=results['true']['eval'][
                        cv],
                                                                                   y_pred=results['rf'][
                                                                                       'eval'][cv])))

            def logit():
                lr_path = '{}logit_{}_robust_cv_{}.pkl'.format(CV_DIR, '-'.join([str(x) for x in out['n_feats_out']]),
                                                               cv)
                coeff_path = '{}logit_{}_robust_coeff.csv'.format(CV_DIR,
                                                                  '-'.join([str(x) for x in out['n_feats_out']]),
                                                                  cv)

                penalty, C = 'l2', 16
                logger.info('Training Logistic Regression with {} penalty and C of {}'.format(penalty, C))
                robust_scale = RobustScaler(with_centering=True, unit_variance=True).fit(X_dev)
                X_dev_robust = robust_scale.transform(X_dev.astype(np.float64))
                X_eval_robust = robust_scale.transform(X_eval.astype(np.float64))
                models['lr'].append(
                    LogisticRegressionCV(penalty=penalty, tol=1e-4, Cs=C, max_iter=5000, solver='newton-cg',
                                         class_weight='balanced', cv=3,
                                         n_jobs=-1, verbose=0,
                                         scoring=make_scorer(matthews_corrcoef),
                                         random_state=0).fit(X=X_dev_robust, y=y_dev))
                results['lr']['dev'].append(models['lr'][cv].predict(X_dev_robust))
                results['lr']['eval'].append(models['lr'][cv].predict(X_eval_robust))
                try:
                    pd.Series(models['lr'][-1].coef_, index=X_dev.columns).sort_values(ascending=False).to_csv(
                        coeff_path)
                except:
                    logger.info('No coef_ today.')
                with open(lr_path, 'w+b') as f:
                    pickle.dump((models['lr'][0], (indices['dev'][0], indices['eval'][0])), file=f)

            def run_perceptron(version=2):
                mlp_path = '{}deep_mlp_{}_normed_cv_{}.pkl'.format(CV_DIR,
                                                                   '-'.join([str(x) for x in out['n_feats_out']]),
                                                                   cv)
                ...
                X_dev_under, y_dev_under = NearMiss(version=2, n_jobs=-1).fit_resample(X_dev_normed, y_dev)
                models['mlp'].append(MLPClassifier(random_state=0, hidden_layer_sizes=(50, 75, 100, 75, 50),
                                                   learning_rate='invscaling').fit(X_dev_under, y_dev_under))
                if cv == 0:
                ...
                try:
                    logger.info('Results for (50, 75, 100, 75, 50) MLP: Normed NearMiss-{}'.format(version))
                    logger.info('Matthews Correlation Coefficient (Dev Under-Sampled): {}:'.format(matthews_corrcoef(
                        y_true=norm_under_dev_tup[0],
                        y_pred=norm_under_dev_tup[1])))
                    logger.info('Matthews Correlation Coefficient (Dev): {}:'.format(matthews_corrcoef(
                        y_true=normed_all_dev_tup[0], y_pred=normed_all_dev_tup[1])))
                    logger.info('Matthews Correlation Coefficient (Eval): {}:'.format(matthews_corrcoef(
                        y_true=normed_all_eval_tup[0], y_pred=normed_all_eval_tup[1])))
                    logger.info('Balanced Accuracy Score (Dev Under): {}:'.format(balanced_accuracy_score(
                        y_true=norm_under_dev_tup[0],
                        y_pred=norm_under_dev_tup[1])))
                    logger.info('Balanced Accuracy Score (Dev): {}:'.format(
                        balanced_accuracy_score(y_true=normed_all_dev_tup[0], y_pred=normed_all_dev_tup[1])))
                    logger.info('Balanced Accuracy Score (Eval): {}:'.format(balanced_accuracy_score(
                        y_true=normed_all_eval_tup[0], y_pred=normed_all_eval_tup[1])))
                except:
                    print('MLP scoring error')
                with open(mlp_path, 'w+b') as f:
                    pickle.dump((models['mlp'][cv], (indices['dev'][cv], indices['eval'][cv])), file=f)
            ...
            for cv in list(range(5)):
                ...
                X_dev_all_arr, y_dev_arr = check_X_y(X=std_cut_feats.iloc[indices['dev'][cv]],
                                                     y=fs.y.iloc[indices['dev'][cv]])
                X_eval_all_arr, y_eval_arr = check_X_y(X=std_cut_feats.iloc[indices['eval'][cv]],
                                                       y=fs.y.iloc[indices['eval'][cv]])
                X_eval_all = pd.DataFrame(data=X_eval_all_arr, index=std_cut_feats.index[indices['eval'][cv]],
                                          columns=std_cut_feats.columns)
                y_eval = pd.Series(data=y_eval_arr, index=fs.y.iloc[indices['eval'][cv]])
                X_dev_all = pd.DataFrame(data=X_dev_all_arr, index=std_cut_feats.index[indices['dev'][cv]],
                                         columns=std_cut_feats.columns,
                                         dtype=np.float32)
                ...
                weights['dev'].append(compute_sample_weight(class_weight='balanced', y=y_dev))
                weights['eval'].append(compute_sample_weight(class_weight='balanced', y=y_eval))
                ...
                if mod_name == "lr":
                    ...
                    run_perceptron()
        try:
            plot_results(results_dict=results, models=models, indices=indices, weights=weights,
                         insol_label=insol_label, feats=feats_df_list, save_dir=RUN_DIR)
        except:
