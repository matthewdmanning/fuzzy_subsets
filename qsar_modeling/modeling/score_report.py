...
from sklearn.metrics import RocCurveDisplay, ConfusionMatrixDisplay, get_scorer_names, get_scorer, DetCurveDisplay
        ...
        if mod_name != 'true':
            for s, split_name in enumerate(mod.keys()):
                ...
                for cv_num in range(len(mod[split_name])):
                    score_kwargs = {'sample_weight': weights[split_name][cv_num], 'pos_label': 0,
                                    'average':       'weighted', 'adjusted': 'False'}
    rocfig, rocax = plt.subplots()
    rocfig.set_dpi(600)
    return rocfig, rocax


def get_confusion_display(estimator, X, y_true, class_names=None, axes=None, colors=None):
    if colors is None:
        colors = plt.get_cmap('coolwarm').reversed(name='warmcool')
    cmd = ConfusionMatrixDisplay.from_estimator(X=X, y=y_true, estimator=estimator, ax=axes, display_labels=class_names,
                                                cmap=colors, xticks_rotation=0.3, normalize='true',
                                                values_format='.4f')
    return cmd


def roc_bounds(model_dict, fig, ax, label_insol, results_dict, estimators, feat_df, plot_cvs=False):
    ...
    for j, mod_name in enumerate(model_dict.keys()):
        ...
        std_auc = np.std(aucs)
        print('{}: Mean AUC: {}. StdDev AUC: {}'.format(names_dict[mod_name], mean_auc, std_auc))
        ax.plot(
            mean_fpr, mean_tpr, color=mod_colors[j], lw=1.5, alpha=0.8,
            label=r"%s Mean ROC (AUC = %0.3f $\pm$ %0.3f)" % (names_dict[mod_name], mean_auc, std_auc)
        )
        if plot_cvs:
            for i, disp in enumerate(disp_list):
                disp_list.plot(ax=ax, name='{}: CV {}'.format(names_dict[mod_name], i), alpha=0.2, lw=1,
                               color=mod_colors[j])
        std_tpr = np.std(tprs, axis=0)
        tprs_upper = np.minimum(mean_tpr + std_tpr, 1)
        tprs_lower = np.maximum(mean_tpr - std_tpr, 0)
        ax.fill_between(
            mean_fpr, tprs_lower, tprs_upper, color=mod_colors[j], alpha=0.2, label=r"$\pm$ 1 Std. Dev."
        )
...
def plot_results(results_dict, indices, models, feats, weights=None, insol_label=0, save_dir=None):
    mod_colors = ['b', 'r', 'g']
    ...
    index_multi = pd.MultiIndex.from_product(iterables=[[k for k in results_dict.keys() if k != 'true'],
                                                        scores.keys()],
                                             names=['Model', 'Metric'])
    col_multi = pd.MultiIndex.from_product(iterables=[['Mean', 'Std'], ['dev', 'eval']], names=['Stat', 'Split'])
    ...
    for m, (mod_name, mod) in enumerate(results_dict.items()):
        ...
        for s, split_name in enumerate(mod.keys()):
            ...
            for metric, fnc in score_dict.items():
                score_list = list()
                for cv_num in range(len(mod[split_name])):
                    score_kwargs = {'sample_weight': weights[split_name][cv_num], 'pos_label': insol_label,
                                    'average':       'weighted', 'adjusted': 'False'}
                    rel_kwargs = dict([(k, v) for k, v in score_kwargs.items() if k in inspect.getfullargspec(fnc)])
                    score_list.append(fnc(y_true=results_dict['true'][split_name][cv_num],
                                          y_pred=results_dict[mod_name][split_name][cv_num], **rel_kwargs))
                score_df.loc[(mod_name, metric), ('Mean', split_name)] = np.mean(score_list)
                if len(score_list) > 1:
                    score_df.loc[(mod_name, metric), ('Std', split_name)] = np.std(score_list)
                else:
            ...
            for cv_num in range(len(mod[split_name])):
                ...
                if split_name == 'eval':
                    roc_dict[mod_name] = [RocCurveDisplay.from_estimator(estimator=models[mod_name][cv_num],
                                                                         X=feats.iloc[
                                                                           indices[split_name][cv_num], :],
                                                                         y=results_dict['true'][split_name][
                                                                             cv_num],
                                                                         sample_weight=weights[split_name][
                                                                             cv_num],
                                                                         pos_label=insol_label,
                                                                         name=names_dict[mod_name]) for
                                          cv_num in list(range(len(models[mod_name])))]
                    det_dict[mod_name] = [DetCurveDisplay.from_estimator(estimator=models[mod_name][cv_num],
                                                                         X=feats.iloc[
                                                                           indices[split_name][cv_num], :],
                                                                         y=results_dict['true'][split_name][
                                                                             cv_num],
                    ...
                    logging.info(pprint.pformat(
                        'ROC-AUC for {} Eval Set: {:.4f}'.format(mod_name, roc_dict[mod_name][cv_num].roc_auc)))
    ...
    while os.path.isfile(score_path):
        i += 1
    logging.info(pprint.pformat(score_df.to_string(float_format='{:.5f}'), compact=True))
    ...
    try:
        if feats is not None:
            fig, ax = roc_bounds(fig=rocfig, ax=rocax, label_insol=insol_label, estimators=models['roc'],
                                 results_dict=results_dict, model_dict=models,
                                 feat_df=feats)
    except:
        print('Failed to save ROC figure!')
    try:
        rocfig.savefig('{}roc_{}.png'.format(save_dir, '_'.join(roc_dict.keys())), dpi=600, format='png',
                       transparent=True)
        detfig.savefig('{}det_{}.png'.format(save_dir, '_'.join(det_dict.keys())), dpi=600, format='png',
                       transparent=True)
    except:
        print('Failed to save ROC figure!')
