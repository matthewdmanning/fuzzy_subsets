...
from sklearn.metrics import average_precision_score, balanced_accuracy_score, brier_score_loss, classification_report, \
    hamming_loss, jaccard_score, log_loss, make_scorer, matthews_corrcoef, \
...
def get_pred_score_funcs(scalar_only=True):
    ...
    pred_scores = [('Balanced Accuracy', balanced_accuracy_score),
...
def get_score_bounds():
    ...
    bounds = dict([('Balanced Accuracy', (0.5, 1.0)),
...
def get_prob_scores_dict(pos_label=0):
    ...
    prob_scores = dict([
        ('Average Precision Macro', partial(average_precision_score, pos_label=pos_label, average='macro')),
        ('Average Precision Micro', partial(average_precision_score, average='micro')),
        ('Average Precision Weighted', partial(average_precision_score, pos_label=pos_label, average='weighted')),
...
def fit_score_model_cv(estimator, feature_df, labels, cv=StratifiedKFold, score_dir=None, save_model=True, scaler=None):
    ...
    for (score_name, score_obj) in get_pred_score_funcs():
        ...
        for dev_X, dev_y, eva_X, eva_y in cv_tools.split_df(feature_df, labels, splitter=cv):
        ...
        for stat_name, stat in zip(['Mean', 'StDev', 'Median', 'Min', 'Max'],
            ...
            dev_score_summary[stat_name].append('{:.5f}'.format(
                stat([[s] for s in cv_dev_score_dict[score_name]])))
            eva_score_summary[stat_name].append('{:.5f}'.format(
                stat([[s] for s in cv_eval_score_dict[score_name]])))
    dev_df = pd.DataFrame.from_dict(data=dev_score_summary, orient='index',
                                    columns=['Mean', 'StDev', 'Median', 'Min', 'Max'])
    eva_df = pd.DataFrame.from_dict(data=eva_score_summary, orient='index',
                                    columns=['Mean', 'StDev', 'Median', 'Min', 'Max'])
...
def score_model(estimator, feature_df, labels, scorer_tuple=None, scalar_only=True, check_bounds=True):
...
def summarize_scores(score_dict_list, check_scores=True):
    ...
    summary_zip = dict(zip(['Mean', 'GeoMean', 'StDev', 'Median', 'Min', 'Max'],
                           [np.mean, stats.geometric_mean, np.std, np.median, np.min, np.max]))
    print(collated_scores)
    for k, v in collated_scores.items():
        if not np.iterable(v) or not all([isinstance(val, numbers.Number) for val in v]):
        ...
        score_summary = defaultdict(None, [(name, s(v)) for name, s in summary_zip.items()])
...
def learn_curve(estimator, name, feature_df, labels, fname_stub=None):
    ...
    lcd = LearningCurveDisplay.from_estimator(estimator, X=feature_df, y=labels, **common_params, ax=ax)
