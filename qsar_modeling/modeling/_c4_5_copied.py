...
def read_testset(testfile):
    ...
    testset = [line.strip().split(",") for line in all_lines]
    return testset
...
def ID3_chooseBestFeatureToSplit(dataset):
    ...
    for i in range(numFeatures):  # 遍历所有特征
        ...
        uniqueVals = set(
            featList
        )  # 将特征列表创建成为set集合，元素不可重复。创建唯一的分类标签列表
        newEnt = 0.0
...
def C45_chooseBestFeatureToSplit(dataset):
    ...
    for i in range(numFeatures):  # 遍历所有特征
        ...
        print("C4.5中第%d个特征的信息增益率为：%.3f" % (i, infoGain_ratio))
...
def CART_chooseBestFeatureToSplit(dataset):
    ...
    for i in range(numFeatures):
        ...
        print("CART中第%d个特征的基尼值为：%.3f" % (i, gini))
        if gini < bestGini:
            bestGini = gini
            bestFeature = i
    return bestFeature
...
def majorityCnt(classList):
    ...
    sortedClassCont = sorted(
        classCont.items(), key=operator.itemgetter(1), reverse=True
    )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if dec_tree == "1":
            ...
            ID3desicionTree = ID3_createTree(
                dataset, labels_tmp, test_dataset=read_testset(testfile)
            )
        ...
        if dec_tree == "2":
            labels_tmp = labels[:]  # 拷贝，createTree会改变labels
            CFourFive = C45_createTree(
                dataset, labels_tmp, test_dataset=read_testset(testfile)
            )
            print("CFourFive:\n", CFourFive)
            treePlotter.C45_Tree(CFourFive)
            testSet = read_testset(testfile)
            print(
                "C4.5_TestSet_classifyResult:\n",
                classifytest(CFourFive, labels, testSet),
            )
            print("---------------------------------------------")
        ...
        if dec_tree == "3":
            labels_tmp = labels[:]  # 拷贝，createTree会改变labels
            CARTdesicionTree = CART_createTree(
                dataset, labels_tmp, test_dataset=read_testset(testfile)
            )
