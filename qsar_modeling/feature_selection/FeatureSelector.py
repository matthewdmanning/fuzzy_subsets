...
class FeatureSelectorTrainer:
    ...
    def fit(self, X, y, uninterpret_kws=None, cov_mat=None):
        if self.run_params.debug:
            self._X = FeatureFrame(
                frame_data=X.sample(n=2500, axis='index').sample(n=100, axis='columns').copy(deep=True),
                options=self.options, logger=self._logger)
            self.options.sample_wts = self.options.sample_wts[self.X.feat_frame.index]
            self._y = y[X.index]
        else:
            self._X = FeatureFrame(frame_data=X.copy(deep=True), options=self.options, logger=self._logger,
                                   cov_mat=cov_mat)
            self._y = y
        if uninterpret_kws is not None:
            self._remove_uninterpretable()
        self._corr = self.X.feat_frame.corrwith(other=self.y, axis='columns',
                                                method=self.selector_params.cov_meth)
    ...
    def _remove_uninterpretable(self):
        self._uninterpret_feats = pd.Index(
            [feat for feat in self.X.feat_frame.columns if len([kw for kw in self._uninterpret_kws if kw in feat]) > 0])
    ...
    def seq_linear_select(self, df=None, label=None, method='auto', seq_linear_select_scoring='balanced_accuracy',
        ...
        if method == 'elastic' or (method == 'auto' and df.shape[1] < 50):
            seq_linear_select_estimator = partial(ElasticNetCV, n_jobs=self.run_params.pjobs, random_state=0,
                                                  max_iter=5000,
                                                  tol=1e-5, selection='random',
                                                  cv=sklearn.model_selection.GroupKFold(n_splits=3))
        else:
            seq_linear_select_estimator = partial(LassoCV, fit_intercept=False, max_iter=2500, tol=1e-5,
