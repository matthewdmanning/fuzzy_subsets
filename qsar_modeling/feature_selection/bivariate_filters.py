...
def filter_covariants(
    ...
    if sort_ser is not None:
        cov_matrix = cov_matrix[
            sort_ser[feature_df.columns].sort_values(ascending=False).index
        ]
    ...
    for feat in col_list:
        ...
        if c >= n_features_out:
            break
    return (
        feature_df.columns.symmetric_difference(pd.Index(col_list)).tolist(),
        cov_matrix,
    )


def filter_mi_interactions(
    feature_df,
    labels,
    n_features_out,
    univariate_mi,
    n_neighbors=15,
    univariate_quintile=0.75,
    bivariate_mi=None,
):
    ...
    for i, feat in enumerate(col_list):
        ...
        mi = (
            ...
            / univariate_mi.iloc[i]
        )
        self_info = mutual_info_regression(
            feature_df[feat].to_frame(),
            feature_df[feat],
            n_neighbors=n_neighbors,
            random_state=0,
            n_jobs=-1,
