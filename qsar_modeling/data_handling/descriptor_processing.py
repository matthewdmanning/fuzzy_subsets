...
def extract_cached_desc(qsar_ready_df, cached_df):
    ...
    for col, qsar_col in itertools.product(upper_cache_cols, ['SMILES_QSAR', 'SMILES_CANONICAL']):
        ...
        if len(matches.tolist()) > last_num:
            ...
            best_match = matches
    smiles_matches = pd.Index(qsar_ready_df['SMILES_QSAR'].squeeze().tolist()).intersection(cached_df.index)
    canon_matches = pd.Index(qsar_ready_df['SMILES_CANONICAL'].squeeze().tolist()).intersection(cached_df.index)
    if last_num == 0:
        return pd.DataFrame([]), None
    elif len(smiles_matches) > last_num and len(canon_matches):
        return cached_df.loc[smiles_matches].rename(index=map_cols(qsar_ready_df, 'SMILES_QSAR', 'INCHI_KEY'),
                                                    inplace=True), 'SMILES_QSAR'
    elif len(canon_matches) > last_num:
        return cached_df.loc[canon_matches].rename(index=map_cols(qsar_ready_df, 'SMILES_CANONICAL', 'INCHI_KEY'),
                                                   inplace=True), 'SMILES_CANONICAL'
    else:
...
def get_api_desc(desc_path, qsar_df, d_set):
    ...
    for response, smile in grabber.bulk_epa_call(qsar_df['SMILES_QSAR'].tolist()[last_ind:]):
...
def main(data_path=None, qsar_df=None, desc_cache=None, qsar_ready_name=None, cache_path=None, desc_set=None,
         intermetals=False, salt_filter=False):
    """
    ...
    salts, intermetallics = qsar_readiness.salts_and_intermetallics(qsar_df['SMILES_QSAR'].tolist(),
                                                                    intermetallics=intermetals)
