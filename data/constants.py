...
Paths = namedtuple('Paths', (
    'feature_groups', 'combo_path', 'feat_label_tup', 'train', 'feature_frame', 'mutual_info', 'cov_mat'),
                   module=__name__)

# Instance -> System/Model Dependent
paths = Paths('{}padel/padel_desc_groups.csv'.format(FINAL_DIR),
...
Names = namedtuple('Names', ('feat_cols',
                             'auto_corr_weights',
                             'index_groups',
                             'uninterpretable'), module=__name__)
names = Names(('Type', 'Number', 'Descriptor', 'Class'),
              ('mass', 'charges', 'van der Waals', 'Sanderson', 'electronegativites', 'polarizabilities',
               'ionization', 'I-state'),
              ('zero_var', 'nonzero_var', 'sparse', 'dense', 'discrete', 'cont', 'nonneg', 'neg'),
              ('information', 'eigen', 'e-state', 'spectral', 'barysz', 'randic', 'bcut'))

min_feats_dict = {'min_sparse': 10, 'min_dense': 10, 'min_continuous': 20, 'min_discrete': 10}
thresholds_dict = {'cov_thresh': 0.85, 'sparse_thresh': 0.025, 'discrete_thresh': 0.0025}
tolerances_dict = {'tol_discrete': 0.001, 'tol_sparse': 0.01}
methods_dict = {'cov_meth': 'spearman'}
SelectorParams = namedtuple('SelectorParams', (
    *min_feats_dict.keys(), *thresholds_dict.keys(), *tolerances_dict.keys(), *methods_dict.keys()), module=__name__)
selector_params = SelectorParams(*min_feats_dict.values(), *thresholds_dict.values(), *tolerances_dict.values(),
                                 *methods_dict.values())
...
'''
score_dict = {'Balanced Accuracy': balanced_accuracy_score, 'Matthews Correlation': matthews_corrcoef}
names_dict = {'mlp': 'Multilayer Perceptron', 'rf': 'Random Forest', 'lr': 'Logistic Regression', 'eval': 'Eval',
              'dev': 'Dev'}
