...
from qsar_modeling.data_handling.data_tools import get_interpretable, load_training_data, load_metadata
...
def brute_force_importance_rf_clf(feature_df, labels, clf, n_feats_out, step_size=1, n_jobs=-2, verbose=0,
                                  **model_kwargs):
    eliminator = RFE(estimator=clf, n_features_to_select=n_feats_out, step=step_size, verbose=verbose).fit(feature_df,
                                                                                                           y=labels)
    brute_features = pd.Series(eliminator.ranking_, index=feature_df.columns.tolist()).sort_values()
    return brute_features, eliminator


def multistep_rfe_importance(feature_df, labels, clf, steps_feats_tup, n_jobs=-2, verbose=0, **model_kwargs):
    feats = feature_df.columns
    for step, n_feats_out in steps_feats_tup:
        fts, elim = brute_force_importance_rf_clf(feature_df[feats], labels, clf, n_feats_out, step_size=step,
                                                  n_jobs=n_jobs, verbose=verbose, **model_kwargs)
        feats = fts.index
        if verbose:
            pprint.pp(fts)
    return fts, elim
...
full_groups_list = dict(
    [(k, pd.Series(data=k, index=v, name=k)) for k, v in g_dict.items() if k in ['epa_sol', 'epa_in', 'en_in']])
epa_enamine_overlap = full_groups_list['en_in'].index.intersection(full_groups_list['epa_in'].index)
print('EPA-Enamine overlap: \n{}'.format(epa_enamine_overlap))
full_groups_list['en_in'].drop(index=epa_enamine_overlap, inplace=True)
combined_y = interp_y[stepwise_duplicate(pd.concat(full_groups_list.values(), sort=True)).squeeze().index]
...
def averaged_spearman_mutlicollinear(feature_df, labels, save_dir, cv=5, nan_policy='omit'):
    spear_list, ward_list, fig_list = list(), list(), list()
    for i in np.arange(cv):
        feature_rus, labels_rus = RandomUnderSampler(random_state=1000 * i).fit_resample(feature_df, labels)
        feature_rus = feature_rus[feature_rus.var(axis=1) > 1e-4]
        feat_names, spear, dendro, spearfig1, spearfig2 = qsar_modeling.feature_selection.multicollinear.spearman_rank_multicollinear(
    ...
    if avg_spear.isna().count().count() > 0:
        ...
        print(avg_spear)
    feat_names, avg_corr, avg_dendro, avg_fig1, avg_fig2 = multicollinear.spearman_rank_multicollinear(
        feature_df=feature_df[col_list], corr=avg_spear, nan_policy='raise', cluster_feats=True)
    ...
    avg_fig2.dpi = 1000
    avg_fig2.savefig('{}spearman_hierarchical_avg_250_extra_trees.svg'.format(exp_dir), transparent=True)
...
if False:
    ...
    if feature_method == 'brute':
        ...
        xt = ExtraTreesClassifier(n_estimators=1000, random_state=0, n_jobs=-1, class_weight='balanced')
        rfc = RandomForestClassifier(random_state=0, oob_score=True, n_jobs=-1, class_weight='balanced')
        lr = LogisticRegressionCV(Cs=np.geomspace(1e-5, 1., 10), penalty='elasticnet', solver='saga', max_iter=10000,
                                  scoring=make_scorer(matthews_corrcoef), n_jobs=-1, random_state=0, cv=5, tol=1e-3,
                                  l1_ratios=[0.25])
        ranked_features, rfe_model = multistep_rfe_importance(combined_X, combined_y, clf=xt,
                                                              steps_feats_tup=step_tups, verbose=2)
        ...
        indie_features, indie_dendro = averaged_spearman_mutlicollinear(ranked_X, combined_y, exp_dir)
...
for i in np.arange(5):
    ...
    rus_data, rus_labels = RandomUnderSampler(random_state=1000 * i).fit_resample(shuffle_X, shuffle_y)
    epa_rus, epa_labels = RandomUnderSampler(random_state=1000 * i).fit_resample(shuffle_epa_X, shuffle_epa_y)
    ...
    # Even group sampling
    rus_groups = [g[g.index.intersection(rus_labels.index)] for g in full_groups_list.values()]
