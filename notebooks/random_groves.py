...
def get_score_partials(pos_label=None, label_names=None):
    scores = dict(
        [('Balanced Accuracy', balanced_accuracy_score),
         ...
         ('Classification Report', partial(classification_report, target_names=label_names)),
         ('Classification Report - Weighted', partial(classification_report, target_names=label_names)),
         ('Recall Micro', partial(recall_score, pos_label=pos_label, average='micro')),
...
prob_scores = dict([
    ('Average Precision Macro', partial(average_precision_score, pos_label=0, average='macro')),
    ('Average Precision Micro', partial(average_precision_score, pos_label=0, average='micro')),
    ('Average Precision Weighted', partial(average_precision_score, pos_label=0, average='weighted')),
...
def brute_force_importance_rf_clf(feature_df, labels, n_features_out, save_dir, n_jobs=-2, **model_kwargs):
    clf = RandomForestClassifier(random_state=0, oob_score=True, n_jobs=n_jobs, **model_kwargs)
    eliminator = RFE(estimator=clf, n_features_to_select=int(n_features_out)).fit(feature_df, y=labels)
    brute_features = pd.Series(eliminator.ranking_, index=feature_df.columns.tolist()).sort_values()
    brute_features.to_csv('{}feature_rankings.csv'.format(save_dir), index_label='Features', float_format='%.4f')
...
def brute_force_importance_rf_clf(feature_df, labels, n_features_out, save_dir, n_jobs=-2, **model_kwargs):
    clf = RandomForestClassifier(n_estimators=500, random_state=0, n_jobs=n_jobs, **model_kwargs)
    eliminator = RFE(estimator=clf, n_features_to_select=n_features_out).fit(feature_df, y=labels)
    brute_features = pd.Series(eliminator.ranking_, index=feature_df.columns.tolist()).sort_values()
    brute_features.to_csv('{}feature_rankings.csv'.format(save_dir), index_label='Features', float_format='%.4f')
...
def undersampler(feature_df, labels, feature_name, feature_dir):
    ...
    for i in np.arange(1):
        ...
        for dev, eva in StratifiedKFold().split(feature_df, labels):
            ...
            print('Number of insoluble compounds selected: {}'.format(rus_dev_y[rus_dev_y == 0].shape))
            ...
            label_mi = univariate_filters.balanced_mi_y(rus_dev_X, rus_dev_y, n_neighbors=9).iloc[:int(5 * n_feats)]
            ...
            while True:
                hc_feats = correlation_filter.findCorrelation(dev_corr, n_drop=n_drop_feats, cutoff=cut_off, exact=True)
            ...
            final_feats, imp_model = brute_force_importance_rf_clf(rus_dev_X, rus_dev_y, n_feats, cv_dir)
            ...
            label_mi[rus_dev_X.columns].to_csv('{}solubility_features_mutual_info.csv'.format(cv_dir),
            ...
            logistic_cv = partial(LogisticRegressionCV, max_iter=1000, solver='liblinear')
            forest_params = {'n_estimators': [10, 100, 1000], 'max_depth': [5, 10], 'bootstrap': [False],
            ...
            for model_name, model, hypers in model_tup:
                if 'logistic' not in model_name:
                    ...
                    print('MCC score of {} for parameters: {}'.format(search_party.best_score_,
                                                                      search_party.best_params_))
...
for i, ecount in enumerate(estate_counts):
    ...
    under_X, under_sample = RandomUnderSampler(random_state=1000 * i).fit_resample(train_X, e_label)
