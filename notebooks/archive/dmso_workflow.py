...
all_rings = [
    c for c in train_X.columns if "rings" in c.lower() and "membered" in c.lower()
]
path_cols = [
    c
    for c in train_X.columns
    if "Molecular walk count" in c or "Molecular path count" in c
]
bo_cols = [c for c in train_X.columns if "Conventional bond order" in c]
names = {
    "Fused rings": fused,
    "Hetero plain": plain_hetero,
    "Hetero fused": all_rings,
    "Path_Counts_PCA": path_cols,
    "Bond_Oorders_PCA": bo_cols,
}
cols = [
    "Molecular path count of order {}".format(i) for i in range(5, 11)
...
for dev, eva in cv_indices:
    ...
    for feats in pca_replacers:
        ...
        for feat in feats:
            ...
            feature_pca = PCA(n_components=1, whiten=False).fit(
...
fig = px.scatter(
    condition_X.sample(n=1000),
    x="Molecular path count of order 9",
    y="Weiner polarity number",
    color="sol",
    opacity=0.1,
    size_max=0.5,
)
...
sneplot = plt.scatter(
    x=sne_X.iloc[:, 0].to_numpy(),
    y=sne_X.iloc[:, 1].to_numpy(),
    alpha=alpha,
    c=train_y.to_numpy(),
    s=2,
)
