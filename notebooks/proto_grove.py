...
logger_opts = {'filename': "dmso_model.log",
               'filemode': 'w',
...
def get_feature_set(feature_df, labels, split_list, n_features_out):
    ...
    for dev_X, dev_y, eva_X, eva_y in cv_tools.split_df(feature_df.copy(), labels, indices_list=split_list):
        # dev_X = dev_X.iloc[:1000, :]
        # dev_y = dev_y.iloc[:1000]
        mi_feat_list.append(
            get_mi_features(feature_df=dev_X.copy(), labels=dev_y, test_feature_df=eva_X, n_features_out=n_features_out,
                            n_neighbors=7))
        if False:  # os.path.isfile('{}pearson_cov_feature_lists.pkl'.format(os.environ.get('MODELS_DIR'))):
            with open('{}pearson_cov_feature_lists.pkl'.format(os.environ.get('MODELS_DIR')), 'rb') as f:
...
def pca_loop(train_data, test_data, train_labels, n_features_out, sort_key=None, oversampler=True):
    train_data, test_data, train_labels = train_data.copy(), test_data.copy(), train_labels.copy()
    feature_dict = get_features_dict(test_data.columns)
    groups_list = ['Bond_Orders', 'Valence_Path_Counts', 'Molecular_Path_Counts', 'Molecular_Walk_Count', 'Simple_Path',
                   'Self_Return', 'Sized_Rings', 'Total_Rings', 'Rotations',
                   'HBond']
    group_len_dict = dict([(k, len(v)) for k, v in feature_dict.items() if k in groups_list and len(v) < 0])
    train_pca_features, test_pca_features = list(), list()
    for group_name, feat_list in group_len_dict.items():
        present_features = [f for f in feature_dict[group_name] if f in train_data.columns]
        # if sort_key is not None:
        #    feat_list.sort(key=sort_key)
        train_pca_data, test_pca_data, pca_former = feature_combination.pca_threshold(train_data[present_features],
                                                                                      test_data[present_features],
                                                                                      group_name, evr_thresh=0.925,
...
def model_feat_loop(model, model_name, feature_df, labels, feature_set_list, indices_list, pca_features=None):
    model_list, cv_results, wrong_insol, wrong_sol = list(), list(), list(), list()
    print(indices_list)
    print(feature_set_list)
    i = 0
    from imblearn.under_sampling import RandomUnderSampler
    for indices, feature_set in zip(cv_tools.split_df(feature_df, labels, indices_list=indices_list),
                                    feature_set_list):
        ...
        try:
            ...
            dx, dy, ex, ey, former = pca_loop(train_data=rus[feature_set], test_data=erus[feature_set],
                                              train_labels=rus_y, oversampler=False)
            dev_X = former(dev_X)
            eva_X = former(eva_X)
        ...
        pd.Series(dev_X.columns).to_csv('{}{}{}_pca_feats_mi.csv'.format(models_dir, set_name, model_name))
        # pprint.pp(feature_mi)
    print('Score for {} model on {} features'.format(model_name, set_name))
    cv_tools.log_score_summary(cv_tools.score_cv_results(cv_results), grove_log, level=20)
...
def get_feature_stats(feature_X, labels):
    ...
    for feat in estate_counts:
        positive_list[feat] = labels[feature_X[feat] > 0].value_counts(sort=False, normalize=True)[0]
        feat_name = feat.replace('Count of atom-type E-State: ', '')
        estate_list[feat_name] = (labels[feature_X[feat] > 0] == 0).astype(int).sum() / labels.size
        insol_count = feature_X.loc[labels == 0][feat].value_counts(
            sort=False)  # .rename(feat.replace('Count of atom-type E-State: ', ''))
...
estate_weighted = weight_by_feature(feature_df=train_X.copy(), to_norm=estate_counts, norm_by=weighting_desc)
...
for set_name, feat_sets in [('MI_interaction_list', mi_list)]:
    ...
    if set_name == 'Top_MI':
        pca = True
    for mod_name, mod in zip(['Logistic_Regression', 'Balanced_Random_Forest'], [logistic_clf, balanced_forest]):
        model_out, cv_out, miss_insol, miss_sol = model_feat_loop(mod, mod_name, train_X, train_y,
                                                                  feature_set_list=mi_list, indices_list=ind_list,
                                                                  pca_features=None)
        wrong_insol_df = pd.concat([utils.samples.get_sample_info(w) for w in miss_insol], axis=1)
        wrong_sol_df = pd.concat([utils.samples.get_sample_info(w) for w in miss_sol], axis=1)
        wrong_insol_df.to_csv('{}{}{}_incorrect_insol.csv'.format(models_dir, set_name, mod_name))
        wrong_sol_df.to_csv('{}{}{}_incorrect_sol.csv'.format(models_dir, set_name, mod_name))
exit()
'''
