...
def get_feat_mi(input_df, input_labels):
    ...
    for f in input_df.columns:
        disc_dict[f] = [np.all([a, b]) for a, b in
                        zip(distributions.is_discrete(input_df).tolist(), distributions.is_low_cardinal(input_df),
                            strict=False)]
    ...
    for _ in list(range(30)):
        ...
        mi_list.append(
            pd.Series(mutual_info_classif(rus_X, rus_y, discrete_features=disc_ser, random_state=0, n_jobs=-1),
                      index=rus_X.columns).T)
...
def get_pca_vector(train_data, test_data, train_labels, feature_list, group_name, mi_ser=None, evr_thresh=0.925,
                   mi_tol_pct=5):
    # mi_dict[remain_name] = list()
    over_X, over_y = RandomOverSampler(random_state=0).fit_resample(train_data, train_labels)
    ordered_feats = features.sort_ordinals(feature_list, start=10, step=1)
    subset = ordered_feats[:2]
    for feat in ordered_feats[2:]:
        subset.append()
        this_PCA = PCA(n_components=1, whiten=False, random_state=0).fit(X=over_X[subset], y=over_y)
        logger.info('Features in {}'.format(remain_name))
        [logger.info('Mean MI: {} for {}'.format(top_mi_ser[f], f)) for f in feats]
        logger.info('Explained Variance Ratios: {}'.format(pd.Series(this_PCA.explained_variance_ratio_).to_string()))
        logger.info('Principal Components: {}'.format(this_PCA.components_))
        pca_dev_X = pd.DataFrame(this_PCA.transform(train_data[feats].copy()), columns=[remain_name],
                                 index=train_data.index)
        pca_eval_X = pd.DataFrame(this_PCA.transform(test_data[feats].copy()), columns=[remain_name],
...
bad_feats = pd.Index([c for c in train_X if any([u.lower() in c.lower() for u in constants.names.uninterpretable])])
...
listed_top_feats = ['Molecular path count of order 6',
                    ...
                    'Number of rings', 'Conventional bond order ID number of order 8 (ln(1+x)',
...
for cv, (split_dev_X, original_dev_y, split_eval_X, original_eval_y) in enumerate(split_data):
    original_dev_X, original_eval_X = split_dev_X.copy(), split_eval_X.copy()
    if False:  # os.path.isfile('{}rus30_mi_cv{}.csv'.format(os.environ.get('MODELS_DIR'), cv)):
        # mi_df = pd.read_csv('{}rus30_mi_cv{}.csv'.format(os.environ.get('MODELS_DIR'), cv))
        mi_df = pd.read_csv('balanced_mi_all_train_cv30.csv'.format(os.environ.get('MODELS_DIR'), cv))
    else:
    ...
    for n_feat in list(range(top_mi_ser.shape[0])):
        print('Feature selected with MI of {:.5f}: {}'.format(top_mi_ser[n_feat], top_mi_ser.index[n_feat]))
    ...
    for remain_name in pca_replacers:
        feats = [f for f in names[remain_name] if f in replacers_list[0].index or replacers_list[0].columns]
        print(feats)
        pca_vecs, dropped_feats = get_pca_vector(original_dev_X, original_eval_X, original_dev_y,
                                                 group_name=remain_name, feature_list=feats, mi_ser=mi_df)
    ...
    eval_remain = pd.concat([original_eval_X[top_feats].copy(), *pca_dict['eval']], axis=1)
    logger.info('Mutual Information for original and PCA transformed features. Discrete used for original path counts.')
    ...
    rf_dev_path = '{}rf_{}_dev_cv{}.csv'.format(os.environ.get('MODELS_DIR'), remain_name, cv)
    lr_dev_path = '{}lr_{}_dev_cv{}.csv'.format(os.environ.get('MODELS_DIR'), remain_name, cv)
    rf_eval_path = '{}rf_{}_eval_cv{}.csv'.format(os.environ.get('MODELS_DIR'), remain_name, cv)
    lr_eval_path = '{}lr_{}_eval_cv{}.csv'.format(os.environ.get('MODELS_DIR'), remain_name, cv)
...
lr_predict_tups = [(a[1], b[1], b[2]) for (a, b) in zip(pca_split_data, lr_results_list)]
rbf_predict_tups = [(a[1], b[1], b[2]) for (a, b) in zip(pca_split_data, rbf_results_list)]
...
probs_predict_tups = [(a[1], b[1], b[2]) for (a, b) in zip(pca_split_data, probs_results_list)]
...
[logger.info(jaccard_score(y_true=a[1], y_pred=b[1], pos_label=0, average='weighted')) for a, b in
 zip(rbf_results_list, probs_results_list)]
