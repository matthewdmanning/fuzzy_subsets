...
from data_handling.data_tools import data_by_groups
...
def get_notebook_data():
    ...
    unique_X, unique_y = data_handling.data_cleaning.clean_and_check(interp_X, interp_y, y_dtype=int)
...
def cv_model_documented(input_X, input_y, cv_model, model_name, save_dir, cv_splitter=None, sweight=None,
    ...
    for dev_X, dev_y, eva_X, eva_y in cv_tools.split_df(input_X, input_y, splitter=cv_splitter, **splitter_kw):
        ...
        dev_score_dict, eva_score_dict = scoring.score_model(cv_model, dev_X, eva_X, dev_y, eva_y)
...
def combined_rus_cv_results(feature_df, labels, model, model_params, model_name, save_dir, n_rus=5, cv_splitter=None,
    ...
    for r in np.arange(n_rus):
        ...
        rus_dev_scores, rus_eva_scores = cv_model_documented(X_under, y_under, model_inst, model_name, get_rus_dir,
...
for p_id, (param_set) in enumerate(forest_params):
    ...
    get_dir = partial(get_dir(parent_dir=hyper_path, suffix="{}_param_{}/".format(model_id, p_id)))
    os.makedirs(param_dir, exist_ok=True)
    hyper_dev_df, hyper_eva_df, all_dev_scores, all_eva_scores = combined_rus_cv_results(final_X, final_y, base_model,
                                                                                         param_set, model_name=model_id,
                                                                                         save_dir=get_dir,
                                                                                         sweight=samp_wts)
...
crit_df = pd.DataFrame.from_dict(summary_dict, orient='index').sort_values(by="GeoMean_eval", ascending=False)
