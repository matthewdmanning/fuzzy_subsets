...
def undersampler(
    ...
    **kwargs
    ...
    if type(sampler) is str and os.path.isfile(
        "{}{}_undersamples_ind.pkl".format(subsamples_dir, sampler)
    ):
        with open(
            os.path.isfile("{}{}_undersamples_ind.pkl".format(subsamples_dir, sampler)),
            "rb",
        ) as f:
    ...
    default_samplers = {
        ...
        "NearMiss1": near_miss_one,
        "NearMiss2": near_miss_two,
        "NearMiss3": near_miss_three,
        "CondensedNN": condensed,
    }
    if sampler == "auto":
    ...
    for name, sampler in sampler_dict.items():
        ...
        print(
            "Sampled {} with {} samples out of {}".format(
                name, x_samp.shape[0], scaled_df.shape[0]
            )
        )
...
def cross_val(feat_df, target, sampler, estimator, score_dict, name_dict):
    ...
    logger.info(
        [
            "Dev Scores: {}".format(
                " ".join(["{:.5f}".format(s.train_score) for s in results])
            )
        ]
    )
    logger.info(
        [
            "Eval Scores: {}".format(
                " ".join(["{:.5f}".format(s.test_score) for s in results])
            )
        ]
    )
...
def preprocessing_pipe(fs, feat_cols, score_dict, cv_dir, jmem=None, n_jobs=-1):
    ...
    for dev, eval in StratifiedGroupKFold().split(
        X=fs.X.feat_frame, y=fs.y, groups=fs.y
    ):
        ...
        for feature, class_ind in feat_labels.items():
            for sample_method in ["RandomUnderSampler", "NearMiss2"]:
                ...
                sampled, dropped = undersampler(
                    fs.X.feat_frame[pred_feats.extend(feature)],
                    sampler=sample_method,
                    class_labels=class_ind,
                )[sample_method]
                sol_sampled = fs.y[fs.y == sol_label].iloc[sampled]
                insol_sampled = fs.y[fs.y == insol_label].iloc[sampled]
                logger.info(
                    "{} chose {} of {} soluble and {} of {} insoluble compounds".format(
                        sample_method,
                        sol_sampled.size,
                        sol_counts,
                        insol_sampled.size,
